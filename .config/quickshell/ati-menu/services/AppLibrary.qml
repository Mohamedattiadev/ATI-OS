...
Item {
  ...
  function launch(desktopId, name) {
    ...
    // ATI-OS: dropped the "uwsm-app --" prefix Omarchy wraps this in (scopes
    // the launch under app-graphical.slice via UWSM, Universal Wayland
    // Session Manager) -- confirmed via `command -v` that uwsm-app is not
    // installed here; this session doesn't use UWSM at all. gtk-launch
    // itself is present and does the actual desktop-entry resolution
    // (handles ids with spaces, keeps the .desktop suffix or ids like
    // org.telegram.desktop won't resolve), so launching still works
    // correctly -- it just isn't slice-scoped the way Omarchy's is.
