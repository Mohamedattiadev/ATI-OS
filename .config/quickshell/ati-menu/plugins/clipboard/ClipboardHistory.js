// Vendored from Omarchy's shell/plugins/clipboard/ClipboardHistory.js (MIT,
// see ../../../NOTICE.md), modified in two places (search "ATI-OS" below):
// normalizeEntry now keeps a text entry's capturedAt instead of discarding
// it (upstream only ever set this on image entries, since its own
// capture.sh never wrote one onto a text entry either -- ati-clip-capture
// does), and displayRows carries capturedAt through to the row the QML
// picker renders. Everything else -- dedup key, file:// URI detection,
// the display-text size cap -- is unmodified.
function normalizeEntry(value) {
  ...
  if (type === "text") {
    ...
    // ATI-OS: text entries can carry a capturedAt now too (see
    // ati-clip-capture's emit_text) -- upstream only ever checked this on
    // the image branch below.
    if (value.capturedAt !== undefined && value.capturedAt !== null)
      textEntry.capturedAt = String(value.capturedAt)
    return textEntry
  }
...
// shell. Pasting reads the full entry back from history by index, so
// nothing is lost.
...
function cappedEntry(entry) {
  ...
  return { type: "text", text: entry.text.slice(0, cut > 0 ? cut : displayTextLimit), capturedAt: entry.capturedAt }
}
...
function displayRows(history, query, limit) {
  ...
  for (var i = 0; i < values.length; i++) {
    ...
    rows.push({
      ...
      // ATI-OS addition: carried through to the row so the preview pane
      // can show "Copied <when>" for every entry, not just images.
