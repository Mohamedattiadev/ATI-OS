...
Item {
  ...
  // omarchyPath/shell/manifest were injected by omarchy-shell when this
  // plugin was summoned as part of Omarchy's own shell. Ported here as a
  // standalone entry point (see ati-menu/shell.qml) instead: shell/manifest
  // are read in exactly one place below (root.appLibrary, already null-safe
  // for a missing shell) and never assigned, so they stay null harmlessly.
  ...
  //
  // Pointed at ATI-OS's OWN data files (same ones ati-menu, the rofi-based
  // engine this replaces, already reads) rather than a separate JSONC pair
